...
import {encode} from './login/base64'
const { SEI_GLOBAL} = utils;
import { login as loginService, getSizeLimit } from './login/service'
import {httpUtils} from './FeatchUtils'
const { seiIntl  } = seiLocale;

export function login() {
  const user = {
    password: encode('123abc'),
    account: encode('svolt')
  }
  loginService(user).then(res => {
    ...
    if (res.success && res.data) {
      if (res.data.loginStatus === "success") {
        ...
        console.error(seiIntl.get({key: 'gwmBasic_000154', desc: '登录时需要传入租户代码'}));
      } else if (res.data.loginStatus === "captchaError") {
        console.error(seiIntl.get({key: 'gwmBasic_000155', desc: '验证码错误'}));
      } else if (res.data.loginStatus === "frozen") {
        console.error(seiIntl.get({key: 'gwmBasic_000156', desc: '账号被冻结'}));
      } else if (res.data.loginStatus === "locked") {
        console.error(seiIntl.get({key: 'gwmBasic_000157', desc: '账号被锁定'}));
      } else if (res.data.loginStatus === "failure") {
        console.error(seiIntl.get({key: 'gwmBasic_000158', desc: '账号密码错误或账号不存在'}));
      } else {
        console.error(seiIntl.get({key: 'gwmBasic_000159', desc: '登录失败'}));
      }
    } else {
      console.error(seiIntl.get({key: 'gwmBasic_000159', desc: '登录失败'}));
    }
...
export const formatDate=(numb, format = "-")=> {
    ...
    // return  dateStr;
    //王泽南修改
    return moment('1900-1-1').add(numb-2, 'days').format('YYYY-MM-DD');
}
//将map转换成数组
...
//to方法，用于捕捉asyng await的错误,使用时传入一个promise即可
