...
import {Input, Icon, Button, Col, Row, Spin} from 'antd';
...
class SearchTableNew extends Component {
  params = null
  value = ''
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.value && typeof nextProps.value === 'string' && nextProps.value.indexOf(',') === -1) {
      if (this.state.value !== nextProps.value&&nextProps.textValue) {
        this.setState({value: nextProps.value, textValue: nextProps.textValue});
      }
    ...
    //检测config里的service变化，变化重新请求
    if(nextProps.config&&nextProps.config.dataService&&
        nextProps.config.dataService!==this.props.config.dataService){
        if (this.props.initValue) {
            this.getInitData(null,nextProps.config.dataService)
        } else {
            this.getDataSource(null,null,nextProps.config.dataService)
        }
    }
    if (nextProps.params &&
      this.props.params &&
  ...
  componentWillMount() {
    if (!this.props.value) {
      this.setState({textValue: '', value: ''})
    }else if(this.props.value&&this.props.textValue){
        this.setState({value: this.props.value, textValue: this.props.textValue});
    }
    //如果是新增模式预先加载数据
    //进入组件先加载数据
      if(this.props.isAdd) {
  ...
  getInitData(value,dataService) {
    dataService = dataService||this.props.config.dataService;
    ...
    dataService(requestParams).then((res) => {
      let list;
      if(res&&res.data){
        res = res.data;
      }
  ...
  getDataSource(value, pageInfo,dataService) {
    dataService = dataService||this.props.config.dataService;
    const {key, text} = this.props.config;
  ...
  dropDown = () =>{
    const {columns} = this.props.config
    let {filterData,dataLoaded} = this.state;
    //没有加载数据
    if(!dataLoaded){
        //如果是分页查询，拼装下参数
        ...
        if (!this.props.config.searchService){
         ...
         this.props.config.dataService(requestParams).then((res) => {
            ...
            this.setState({dataSource: res,filterData: res,dataLoaded:true})
            })
    }
    //加载样式暂未定
      // !dataLoaded?<Spin tip="Loading...">
      // </Spin>:
