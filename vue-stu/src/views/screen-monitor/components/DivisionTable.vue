<template>
  <VisualTable :tableData="data" height="100%" 
  class="division-table-wrap" v-bind="$attrs">
  ...
  updated() {
    setTimeout(() => {
      ...
      if (cutHead.clientWidth !== 0) {
        rotateDiv.style.transform = `rotate(${
          (Math.atan(cutHead.clientHeight / cutHead.clientWidth) * 180) /
          Math.PI
        }deg)`;
      }
    }, 50);
