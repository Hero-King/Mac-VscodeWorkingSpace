          ...
          <div class="flex w100 spaceBetween">
            <div class="table-item">
              <VisualTable
                ...
                :tableData="EfficiencyStepTableData"
                height="100%"
              >
                <el-table-column
                  label="工序"
                ...
                >
                  <template slot-scope="scope">
                    <img
                      class="arrow-img"
                      :src="
                        scope.row['isGoodRateUp'] ? UpArrowImg : DownArrowImg
                      "
                      alt=""
                      srcset=""
                      height="18px"
                ...
                <el-table-column label="环比" min-width="15%" align="center">
                  <template slot-scope="scope">
                    <img
                      class="arrow-img"
                      :src="
                        scope.row['isOeeRateUp'] ? UpArrowImg : DownArrowImg
                      "
                      alt=""
                      srcset=""
                      height="18px"
...
import {
  queryType,
  getEfficiencyStep,
  getEfficiencyBottleneck,
  getRecentlyYieldOee,
  getEfficiencyTrend,
} from "@/api/screen-monitor/efficiencyMonitor";
...
export default {
  ...
  computed: {
    ...
    procedureNameSet() {
      return Array.from(
        new Set(this.EfficiencyBottleneck.map((i) => i["procedure_name"]))
      );
    },
    monthPickerDisabled() {
    ...
    filterDataByLine() {
      return this.filterDataByFactory.filter(
        (i) => i["line"] === this.formParam.line
      );
    },
    allLine() {
      return Array.from(
        new Set(this.filterDataByFactory.map((i) => i["line"]))
      );
    },
    allProductType() {
    ...
    filterDataByFactory() {
      return this.allTypeData.filter(
        (i) => i["factory"] === this.formParam.factory
      );
    },
    allFactory() {
    ...
    param() {
      ...
      return {
        ...
        start_month: month
          ? this.formParam.year - 1 + this.formParam.month
          : "",
  ...
  methods: {
    ...
    getData() {
      ...
      getRecentlyYieldOee({
        ...this.param,
        grouptype:
          this.formParam.product.length === 0
            ? 1
            : this.formParam.product.length == 1
            ? 2
            : 4,
      })
        .then((res) => {
          this.RecentlyYieldOee = this.parseAjaxData(res.data).sort(
            (a, b) => a["month_code"] - b["month_code"]
          );
      ...
      getEfficiencyBottleneck(productParam)
        .then((res) => {
          ...
          this.EfficiencyBottleneck = this.parseAjaxData(res.data).sort(
            (a, b) => a["month_code"] - b["month_code"]
          );
        })
        .catch((err) => {
      ...
      getEfficiencyStep(productParam)
        .then((res) => {
          ...
          res.data.forEach((i) => {
            i["isGoodRateUp"] =
              (i["good_rate_parent"] || 0) - (i["good_rate_parent_lm"] || 0) >
              0;
            i["isOeeRateUp"] = (i["oee_oee"] || 0) - (i["oee_oee_lm"] || 0) > 0;
            i["good_rate_parent"] =
              this.formatRate(i["good_rate_parent"], 1) + "%";
          ...
          this.EfficiencyStepTableData = res.data.sort(
            (a, b) => a["procedure_code"] - b["procedure_code"]
          );
        })
        .catch((err) => {
