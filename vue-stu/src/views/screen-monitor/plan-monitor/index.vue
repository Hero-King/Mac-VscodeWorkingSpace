    ...
    <div class="plan-body flex">
      <div class="left-wrap h100 flexCol">
        <BasicCard title="生产总览" class="body-line-card left-card">
          <PercentBoard
            :percentCtx="productionPreview.qty.rate"
            :numBottomCtx="productionPreview.qty.act"
            :numTopCtx="productionPreview.qty.plan"
          />
          <PercentBoard
            :percentCtx="productionPreview.cap.rate"
            :numBottomCtx="productionPreview.cap.act"
            :numTopCtx="productionPreview.cap.plan"
          />
        </BasicCard>
      ...
      <div class="right-wrap h100 flexCol">
        ...
        <BasicCard title="计划完成率明细" class="body-line-card table-wrap">
          <VisualTable
            class="monitor-small-table"
            row-class-name="monitor-table-row"
            :tableData="ProductMaterialdetail"
            height="100%"
          >
            <el-table-column
              label="工厂"
...
import {
  ...
  getProductByModel,
  getProductByBase,
  getProductMaterialdetail,
} from "@/api/screen-monitor/planMonitor";
  ...
  methods: {
    ...
    getData() {
      ...
      getProductByBase(this.param)
        .then((res) => {
          this.ProductByBase = this.parseAjaxData(
            res.data.sort((a, b) => a["show_col"] - b["show_col"])
          );
