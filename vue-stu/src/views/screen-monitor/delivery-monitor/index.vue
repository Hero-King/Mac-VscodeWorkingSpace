    ...
    <div class="delivery-body flexCol">
      <div class="body-line">
        <BasicCard title="交付纵览" class="body-line-card left-card">
          <PercentBoard
            :percentCtx="deliveryPreview.qty.rate"
            :numBottomCtx="deliveryPreview.qty.act"
            :numTopCtx="deliveryPreview.qty.plan"
          />
          <PercentBoard
            :percentCtx="deliveryPreview.cap.rate"
            :numBottomCtx="deliveryPreview.cap.act"
            :numTopCtx="deliveryPreview.cap.plan"
          />
        </BasicCard>
      ...
      <div class="body-line">
        ...
        >
          <VisualTable
            ...
            :tableData="DeliveryDetailByFactoryAndProduction"
            height="100%"
          >
            <el-table-column
              ...
              <template slot-scope="scope">
                <div class="flex spaceAround">
                  ...
                  <Dot
                    width="2.4vh"
                    v-show="
                      scope.row['gap_delivery_qty'] &&
                      scope.row['gap_delivery_qty'].toString().length > 0
                    "
                    :color="
                      scope.row['stock_qty'] >= scope.row['wait_delivery_qty']
                        ? '#1AF476'
                        : scope.row['stock_qty'] /
                            scope.row['wait_delivery_qty'] >
                          0.9
                        ? '#F5A210'
                        : '#F53010'
                    "
  ...
  computed: {
    ...
    pieData() {
      ...
      if (this.DeliveryDetailByProduction.length > 0) {
        const [filterData, lessPart, lessPartTotal] = this.filterDataToOther(
          this.DeliveryDetailByProduction,
          "act_delivery_qty",
          reduceToOtherPercent
        );
    ...
    getData() {
      ...
      getDeliveryByProduction(this.param)
        .then((res) => {
          ...
          // 只有实际交付数量 > 0 的产品才显示
          this.DeliveryDetailByProduction = this.parseAjaxData(
            res.data.filter(
              (i) => i["act_delivery_qty"] && i["act_delivery_qty"] > 0
            )
          );
        })
        .catch((err) => {
      ...
      getDeliveryByCustomer(this.param)
        .then((res) => {
          ...
          this.DeliveryDetailByCustomer = res.data.filter(
            (i) => i["act_delivery_qty"] && i["act_delivery_qty"] > 0
          );
        })
        .catch((err) => {
      ...
      getDeliveryByFactoryAndProduction(this.param)
        .then((res) => {
          res.data.forEach((i) => {
            if (
              i.hasOwnProperty("delivery_rate") &&
              i["delivery_rate"] !== "-"
            ) {
